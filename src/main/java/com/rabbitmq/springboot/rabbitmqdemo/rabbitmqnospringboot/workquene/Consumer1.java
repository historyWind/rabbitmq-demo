 ...
 *  这样会造成重复消费
 *
 * 关于手动确认和自动确认两者的介绍： https://www.cnblogs.com/niceyoo/p/11454998.html
...
public class Consumer1 {
    public static void main(String[] args) throws IOException {
        ...
        // 消息被确认后，消息就会从队列中移除，没有被确认的消息一直存在队列中，这样可以保证消息的不丢失
        channel.basicConsume("",false,new DefaultConsumer(channel){
            ...
            public void handleDelivery(String consumerTag, Envelope envelope, AMQP.BasicProperties properties, byte[] body) throws IOException {
                ...
                // 参数1 deliveryTag:该消息的index,手动确认消息标识
                // 参数2 multiple：是否批量.false只确认该delivery_tag的消息，true确认小于该delivery_tag的所有消息
                // 如果某一个消费者 在消费信息后没有进行手动确认，那么在就会在队列中会存在Unacked的消息，
                // 这时当该消费者关闭后，这些unacked的消息会重新被其它消费者进行消费，这样会造成重复消费
